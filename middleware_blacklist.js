...
function startBlacklistMiddleware() {
  ...
  app.use((req, res) => {
    ...
    if (isBlacklisted(cleanIP)) {
      console.warn(`[Blacklist] Petición bloqueada desde IP prohibida: ${cleanIP}`);
      return res.status(403).json({ error: 'Acceso denegado desde esta IP' });
    }

    console.log(`[Blacklist] IP autorizada: ${cleanIP}, reenviando a balanceador (${target})`);

    proxy.web(req, res, { target }, (err) => {
      console.error(`[Blacklist] Error al reenviar al balanceador: ${err.message}`);
      res.status(500).send('Error interno del proxy de blacklist');
    });
  });

  app.listen(5000, () => {
    console.log('[Blacklist] Middleware de filtrado escuchando en el puerto 5000');
  });
}
